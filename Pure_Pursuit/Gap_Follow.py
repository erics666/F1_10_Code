...
class reactive_follow_gap:
	...
	while i <= len(free_space_ranges)-1: # travel through the array
	    if (free_space_ranges[i] > 2) and (gStart == -1): 
		gStart = i
	    elif (free_space_ranges[i] <= 2) and (gStart != -1):
		gEnd = i
		if (gEnd - gStart) > bigG:
		    bigG = gEnd - gStart
	    	    bigGStart = gStart
	...
	minVIndex = ranges.index(minV)	# use minVIndex to find the index of the closest value in a not sorted array
        angle = 0.25/minV
	bub = int(angle/data.angle_increment)	# a number to create the bubble size, it should half the total size
